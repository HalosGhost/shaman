...
%s    Time of sunrise            (%H:%M:%S)
%S    Time of sunset             (%H:%M:%S)
...
When called without the \Opt{-q} flag, \Prog{shaman} will return 0 on success else it will return 1.
However, when called with \Opt{-q}, \Prog{shaman}'s return code will be set from 1 to 9 (1 indicating failure, and each other code representing one of the following groups of weather conditions):
